...
@AndroidEntryPoint
class AstronomiaNivelOchoFragment : Fragment(R.layout.fragment_astronomia_nivel_cero) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        secciones = SeccionesLeccion(binding, sessionRepository.getUsuarioId(), 8)
        ...
        binding.buttonNext.setOnClickListener { val s = viewModel.state.value; when {
            s.enRepaso && s.ultimoRepaso -> findNavController().navigate(R.id.action_nivel_ocho_to_examen)
            ...
            !s.practicaCompletada -> findNavController().navigate(R.id.action_nivel_ocho_to_practica)
            else -> findNavController().navigate(R.id.action_nivel_ocho_to_examen)
        } }
        findNavController().currentBackStackEntry?.savedStateHandle
            ?.getLiveData<IntArray>("astronomia_nivel_ocho_repaso")?.observe(viewLifecycleOwner) { temas ->
                viewModel.iniciarRepaso(temas.toList())
                findNavController().currentBackStackEntry?.savedStateHandle?.remove<IntArray>("astronomia_nivel_ocho_repaso")
            }
    ...
    private fun render(s: AstronomiaNivelOchoUiState) {
