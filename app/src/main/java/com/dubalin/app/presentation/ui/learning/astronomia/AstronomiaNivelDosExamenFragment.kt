...
@AndroidEntryPoint
class AstronomiaNivelDosExamenFragment : Fragment(R.layout.fragment_astronomia_quiz) {
    private var _binding: FragmentAstronomiaQuizBinding? = null
    private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelDosExamenViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        tints = options.map { it.buttonTintList }; binding.quizToolbar.title = getString(R.string.astronomy_level_two_exam)
        ...
        binding.buttonQuizRetry.setOnClickListener {
            ...
            if (weak.isEmpty()) viewModel.reiniciar() else {
                findNavController().getBackStackEntry(R.id.astronomiaNivelDosFragment).savedStateHandle
                    .set("astronomia_nivel_dos_repaso", weak.map { it - 1 }.toIntArray())
                findNavController().popBackStack(R.id.astronomiaNivelDosFragment, false)
            }
        }
    ...
    private fun render(s: NivelDosExamenUiState) {
        ...
        if (result != null) {
            ...
            binding.resultTitle.setText(if (result.aprobado) R.string.astronomy_level_two_passed else R.string.astronomy_quiz_retry_title)
            ...
            binding.resultDescription.text = if (result.aprobado) getString(R.string.astronomy_level_two_passed_description)
