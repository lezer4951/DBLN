...
typealias NivelSeisExamenUiState = EvaluacionUiState
typealias NivelSeisExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelSeisExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[6],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 6, aciertos, total)
    }
)
