...
@AndroidEntryPoint
class AstronomiaNivelNueveFragment : Fragment(R.layout.fragment_astronomia_nivel_cero) {
    private var _binding: FragmentAstronomiaNivelCeroBinding? = null
    private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelNueveViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        secciones = SeccionesLeccion(binding, sessionRepository.getUsuarioId(), 9)
        tints = options.map { it.buttonTintList }; binding.lessonToolbar.title = getString(R.string.astronomy_level_nine_title)
        ...
        binding.buttonNext.setOnClickListener { val s = viewModel.state.value; when {
            s.enRepaso && s.ultimoRepaso -> findNavController().navigate(R.id.action_nivel_nueve_to_examen)
            ...
            !s.practicaCompletada -> findNavController().navigate(R.id.action_nivel_nueve_to_practica)
            else -> findNavController().navigate(R.id.action_nivel_nueve_to_examen)
        } }
        findNavController().currentBackStackEntry?.savedStateHandle
            ?.getLiveData<IntArray>("astronomia_nivel_nueve_repaso")?.observe(viewLifecycleOwner) { temas ->
                viewModel.iniciarRepaso(temas.toList())
                findNavController().currentBackStackEntry?.savedStateHandle?.remove<IntArray>("astronomia_nivel_nueve_repaso")
            }
    ...
    private fun render(s: AstronomiaNivelNueveUiState) {
    ...
    private fun contenido(state: AstronomiaNivelNueveUiState) = with(binding) {
        val s = state.sesion; lessonSection.text = getString(R.string.astronomy_level_nine_topic, s.numero); lessonTitle.text = s.titulo
        ...
        lessonFormat.text = getString(R.string.astronomy_aster_guided); lessonFormatHint.setText(R.string.astronomy_level_nine_hint)
    ...
    private fun pregunta(s: AstronomiaNivelNueveUiState) {
