...
typealias NivelSietePracticaUiState = EvaluacionUiState

@HiltViewModel
class AstronomiaNivelSietePracticaViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.practica(7),
    true, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.completarPractica(user, 7).map { true }
    }
)
