...
@AndroidEntryPoint
class AstronomiaNivelCuatroFragment : Fragment(R.layout.fragment_astronomia_nivel_cero) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener { val s = viewModel.state.value; when {
            s.enRepaso && s.ultimoRepaso -> findNavController().navigate(R.id.action_nivel_cuatro_to_examen)
            ...
            !s.practicaCompletada -> findNavController().navigate(R.id.action_nivel_cuatro_to_practica)
            else -> findNavController().navigate(R.id.action_nivel_cuatro_to_examen)
        } }
        findNavController().currentBackStackEntry?.savedStateHandle
            ?.getLiveData<IntArray>("astronomia_nivel_cuatro_repaso")?.observe(viewLifecycleOwner) { temas ->
                viewModel.iniciarRepaso(temas.toList())
                findNavController().currentBackStackEntry?.savedStateHandle?.remove<IntArray>("astronomia_nivel_cuatro_repaso")
            }
    ...
    private fun render(s: AstronomiaNivelCuatroUiState) {
    ...
    private fun contenido(state: AstronomiaNivelCuatroUiState) = with(binding) {
        ...
        lessonFormat.text = getString(R.string.astronomy_aster_guided); lessonFormatHint.setText(R.string.astronomy_level_four_hint)
    ...
    private fun pregunta(s: AstronomiaNivelCuatroUiState) {
