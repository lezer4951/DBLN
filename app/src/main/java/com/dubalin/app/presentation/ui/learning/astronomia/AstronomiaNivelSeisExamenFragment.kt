...
@AndroidEntryPoint
class AstronomiaNivelSeisExamenFragment : Fragment(R.layout.fragment_astronomia_quiz) {
    private var _binding: FragmentAstronomiaQuizBinding? = null; private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelSeisExamenViewModel by viewModels(); private var rendering = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) { super.onViewCreated(view, savedInstanceState)
        ...
        binding.quizToolbar.title = getString(R.string.astronomy_level_six_exam); binding.quizToolbar.setNavigationOnClickListener { findNavController().navigateUp() }
        ...
        binding.buttonQuizRetry.setOnClickListener { val weak = viewModel.state.value.resultado?.temasDebiles.orEmpty()
            if (weak.isEmpty()) viewModel.reiniciar() else { findNavController().getBackStackEntry(R.id.astronomiaNivelSeisFragment).savedStateHandle
                .set("astronomia_nivel_seis_repaso", weak.map { it - 1 }.toIntArray()); findNavController().popBackStack(R.id.astronomiaNivelSeisFragment, false) } }
    ...
    private fun render(s: NivelSeisExamenUiState) {
        ...
        if (result != null) { binding.resultIcon.text = if (result.aprobado) "⭐" else "🔬"
            binding.resultTitle.setText(if (result.aprobado) R.string.astronomy_level_six_passed else R.string.astronomy_quiz_retry_title)
            ...
            binding.resultDescription.text = if (result.aprobado) getString(R.string.astronomy_level_six_passed_description)
