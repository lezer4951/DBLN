...
typealias NivelOchoExamenUiState = EvaluacionUiState
typealias NivelOchoExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelOchoExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[8],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 8, aciertos, total)
    }
)
