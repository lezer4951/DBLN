...
typealias NivelTresExamenUiState = EvaluacionUiState
typealias NivelTresExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelTresExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[3],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 3, aciertos, total)
    }
)
