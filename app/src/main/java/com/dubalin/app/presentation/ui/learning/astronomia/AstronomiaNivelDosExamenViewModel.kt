...
typealias NivelDosExamenUiState = EvaluacionUiState
typealias NivelDosExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelDosExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[2],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 2, aciertos, total)
    }
)
