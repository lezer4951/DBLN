...
class AstronomiaNivelTresExamenFragment : Fragment(R.layout.fragment_astronomia_quiz) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) { super.onViewCreated(view, savedInstanceState)
        ...
        binding.buttonQuizRetry.setOnClickListener { val weak = viewModel.state.value.resultado?.temasDebiles.orEmpty()
            if (weak.isEmpty()) viewModel.reiniciar() else { findNavController().getBackStackEntry(R.id.astronomiaNivelTresFragment).savedStateHandle
                .set("astronomia_nivel_tres_repaso", weak.map { it - 1 }.toIntArray()); findNavController().popBackStack(R.id.astronomiaNivelTresFragment, false) } }
    ...
    private fun render(s: NivelTresExamenUiState) {
        ...
        if (result != null) { binding.resultIcon.text = if (result.aprobado) "⭐" else "🔬"
            binding.resultTitle.setText(if (result.aprobado) R.string.astronomy_level_three_passed else R.string.astronomy_quiz_retry_title)
            ...
            binding.resultDescription.text = if (result.aprobado) getString(R.string.astronomy_level_three_passed_description)
