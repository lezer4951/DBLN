...
typealias NivelSieteExamenUiState = EvaluacionUiState
typealias NivelSieteExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelSieteExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[7],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 7, aciertos, total)
    }
)
