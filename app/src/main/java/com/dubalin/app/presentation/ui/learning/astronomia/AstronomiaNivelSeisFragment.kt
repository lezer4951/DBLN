...
@AndroidEntryPoint
class AstronomiaNivelSeisFragment : Fragment(R.layout.fragment_astronomia_nivel_cero) {
    private var _binding: FragmentAstronomiaNivelCeroBinding? = null
    private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelSeisViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        secciones = SeccionesLeccion(binding, sessionRepository.getUsuarioId(), 6)
        tints = options.map { it.buttonTintList }; binding.lessonToolbar.title = getString(R.string.astronomy_level_six_title)
        ...
        binding.buttonNext.setOnClickListener { val s = viewModel.state.value; when {
            s.enRepaso && s.ultimoRepaso -> findNavController().navigate(R.id.action_nivel_seis_to_examen)
            ...
            !s.practicaCompletada -> findNavController().navigate(R.id.action_nivel_seis_to_practica)
            else -> findNavController().navigate(R.id.action_nivel_seis_to_examen)
        } }
        findNavController().currentBackStackEntry?.savedStateHandle
            ?.getLiveData<IntArray>("astronomia_nivel_seis_repaso")?.observe(viewLifecycleOwner) { temas ->
                viewModel.iniciarRepaso(temas.toList())
                findNavController().currentBackStackEntry?.savedStateHandle?.remove<IntArray>("astronomia_nivel_seis_repaso")
            }
    ...
    private fun render(s: AstronomiaNivelSeisUiState) {
    ...
    private fun contenido(state: AstronomiaNivelSeisUiState) = with(binding) {
        val s = state.sesion; lessonSection.text = getString(R.string.astronomy_level_six_topic, s.numero); lessonTitle.text = s.titulo
        ...
        lessonFormat.text = getString(R.string.astronomy_aster_guided); lessonFormatHint.setText(R.string.astronomy_level_six_hint)
    ...
    private fun pregunta(s: AstronomiaNivelSeisUiState) {
