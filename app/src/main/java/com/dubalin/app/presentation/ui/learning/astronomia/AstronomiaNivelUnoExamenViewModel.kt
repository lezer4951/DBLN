...
typealias NivelUnoExamenUiState = EvaluacionUiState
typealias NivelUnoExamenResultado = EvaluacionResultado

@HiltViewModel
class AstronomiaNivelUnoExamenViewModel @Inject constructor(
    session: SessionRepository,
    repository: NivelAstronomiaRepository,
    saved: SavedStateHandle = SavedStateHandle()
) : EvaluacionAstronomiaViewModel(
    CatalogoAstronomia.examenes[1],
    false, saved,
    { aciertos, total ->
        val user = session.getUsuarioId()
        if (user == null) Result.failure(IllegalStateException("Sesión requerida"))
        else repository.registrarEvaluacion(user, 1, aciertos, total)
    }
)
