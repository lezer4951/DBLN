...
@AndroidEntryPoint
class AstronomiaNivelDiezExamenFragment : Fragment(R.layout.fragment_astronomia_quiz) {
    private var _binding: FragmentAstronomiaQuizBinding? = null; private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelDiezExamenViewModel by viewModels(); private var rendering = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) { super.onViewCreated(view, savedInstanceState)
        ...
        binding.quizToolbar.title = getString(R.string.astronomy_level_ten_exam); binding.quizToolbar.setNavigationOnClickListener { findNavController().navigateUp() }
        ...
        binding.buttonQuizRetry.setOnClickListener { val weak = viewModel.state.value.resultado?.temasDebiles.orEmpty()
            if (weak.isEmpty()) viewModel.reiniciar() else { findNavController().getBackStackEntry(R.id.astronomiaNivelDiezFragment).savedStateHandle
                .set("astronomia_nivel_diez_repaso", weak.map { it - 1 }.toIntArray()); findNavController().popBackStack(R.id.astronomiaNivelDiezFragment, false) } }
    ...
    private fun render(s: NivelDiezExamenUiState) {
        ...
        if (result != null) { binding.resultIcon.text = if (result.aprobado) "🏆" else "🔬"
            binding.resultTitle.setText(if (result.aprobado) R.string.astronomy_level_ten_passed else R.string.astronomy_quiz_retry_title)
            ...
            binding.resultDescription.text = if (result.aprobado) getString(R.string.astronomy_level_ten_passed_description)
