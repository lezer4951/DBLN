...
@AndroidEntryPoint
class AstronomiaNivelTresFragment : Fragment(R.layout.fragment_astronomia_nivel_cero) {
    private var _binding: FragmentAstronomiaNivelCeroBinding? = null
    private val binding get() = _binding!!
    private val viewModel: AstronomiaNivelTresViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        secciones = SeccionesLeccion(binding, sessionRepository.getUsuarioId(), 3)
        tints = options.map { it.buttonTintList }; binding.lessonToolbar.title = getString(R.string.astronomy_level_three_title)
        ...
        binding.buttonNext.setOnClickListener { val s = viewModel.state.value; when {
            s.enRepaso && s.ultimoRepaso -> findNavController().navigate(R.id.action_nivel_tres_to_examen)
            ...
            !s.practicaCompletada -> findNavController().navigate(R.id.action_nivel_tres_to_practica)
            else -> findNavController().navigate(R.id.action_nivel_tres_to_examen)
        } }
        findNavController().currentBackStackEntry?.savedStateHandle
            ?.getLiveData<IntArray>("astronomia_nivel_tres_repaso")?.observe(viewLifecycleOwner) { temas ->
                viewModel.iniciarRepaso(temas.toList())
                findNavController().currentBackStackEntry?.savedStateHandle?.remove<IntArray>("astronomia_nivel_tres_repaso")
            }
    ...
    private fun render(s: AstronomiaNivelTresUiState) {
    ...
    private fun contenido(state: AstronomiaNivelTresUiState) = with(binding) {
        val s = state.sesion; lessonSection.text = getString(R.string.astronomy_level_three_topic, s.numero); lessonTitle.text = s.titulo
        ...
        lessonFormat.text = getString(R.string.astronomy_aster_guided); lessonFormatHint.setText(R.string.astronomy_level_three_hint)
        ...
        buttonNext.setText(when { state.enRepaso && state.ultimoRepaso -> R.string.astronomy_retry_exam
            ...
            !state.practicaCompletada -> R.string.astronomy_go_to_practice; else -> R.string.astronomy_go_to_exam })
    }
    private fun pregunta(s: AstronomiaNivelTresUiState) {
