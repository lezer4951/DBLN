...
@AndroidEntryPoint
class AstronomiaNivelUnoPracticaFragment : Fragment(R.layout.fragment_astronomia_quiz) {
    ...
    private fun render(state: NivelUnoPracticaUiState) {
        ...
        binding.quizContent.isVisible = !state.completada
        binding.quizResult.isVisible = state.completada
        if (state.completada) {
            binding.resultIcon.text = "🛰️"
            binding.resultTitle.setText(R.string.astronomy_practice_complete)
            binding.resultScore.text = getString(R.string.astronomy_quiz_score, state.aciertos, state.preguntas.size, state.aciertos * 100 / state.preguntas.size)
            binding.resultDescription.setText(R.string.astronomy_practice_complete_description)
            binding.buttonQuizFinish.setText(R.string.astronomy_go_to_exam)
            binding.buttonQuizRetry.isVisible = false
            return
        }
        ...
        binding.buttonQuizAction.setText(if (state.feedback == null) R.string.astronomy_check_answer else if (state.actual == state.preguntas.lastIndex) R.string.astronomy_finish_practice else R.string.action_next)
    }
